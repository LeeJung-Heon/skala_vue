...
/** 마커 위에 항상 떠 있는 도시 이름 말풍선을 만듭니다. */
const buildLabelContent = (target) => {
  const wrap = document.createElement('div')
  wrap.style.cssText = 'padding:2px 4px;text-align:center;white-space:nowrap;color:#f4f6fb'

  const name = document.createElement('strong')
  name.textContent = target.name
  name.style.cssText = 'display:block;font-size:14px;font-weight:700'

  const region = document.createElement('span')
  region.textContent = target.region ?? ''
...
const renderMap = async (target) => {
  const { Map3DElement, Marker3DInteractiveElement, PopoverElement } = await loadMaps3d()
  ...
  // Marker3DElement 의 label 은 3D 지도에서 텍스트로 그려지지 않아
  // 마커에 고정한 Popover 로 도시 이름을 표시합니다.
  const popover = new PopoverElement({
    open: true,
    positionAnchor: marker,
    lightDismissDisabled: true,
  })
  popover.append(buildLabelContent(target))
